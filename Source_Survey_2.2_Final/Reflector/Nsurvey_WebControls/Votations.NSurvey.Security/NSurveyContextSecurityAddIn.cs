namespace Votations.NSurvey.Security
{
    ...
    /// </summary>
    public class NSurveyContextSecurityAddIn : IWebSecurityAddIn
    {
        ...
        public Control GetAdministrationInterface(Style controlStyle)
        {
            ...
            this._adminTable.Width = Unit.Pixel(600);
        ...
        {
            bool flag = false;
            if (!NSurveyContext.Current.User.Identity.IsAuthenticated)
            {
                return flag;
            }
            if (((NSurveyContext.Current.User.Identity.Name.Length <= 0) || new Voters().HasUserNameVoted(this.SurveyId, NSurveyContext.Current.User.Identity.Name)) && !new Surveys().NSurveyAllowsMultipleSubmissions(this.SurveyId))
            {
                return flag;
            }
            return true;
        ...
        {
            new Survey().UpdateNSurveySecuritySettings(this.SurveyId, ((CheckBox) sender).Checked);
        }
